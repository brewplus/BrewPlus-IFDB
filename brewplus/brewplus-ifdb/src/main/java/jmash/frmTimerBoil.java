...
public class frmTimerBoil extends JInternalFrame {
	...
	public frmTimerBoil(HopTableModel listasteps, int boiltime, String nome) {
		...
		cmdGooglePlay.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					new Utils.BareBonesBrowserLaunch()
							.openURL("https://play.google.com/store/apps/details?id=ixtlanas.brewplus.brewplustimer");
				} catch (Exception ex) {
				}
			}
		});
		...
		cmdTimer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// fc.setLocation((desktopSize.width -
	...
	private void caricaSteps() {
		...
		for (int x = 0; x < listasteps.getRowCount(); x++) {
			if (!listasteps.getRow(x).getUso().equalsIgnoreCase("kettle")) {
				continue;
			}
	...
	private void addStep() {
		if (txtdesc.getText().trim().equalsIgnoreCase(""))
			return;
