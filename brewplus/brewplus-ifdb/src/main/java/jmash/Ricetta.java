...
public class Ricetta extends javax.swing.JInternalFrame {
		...
		mnuItem.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				if (hop != null) {
					...
					hop = ed.get();
					Ricetta.this.hopTableModel.getRow(Ricetta.this.tblHops.getSelectedRow())
							.setAlfaAcidi(hop.getAlfaAcidi());
					ricettaModificata();
				}
			}
		});
	...
	private void initComponents() {
		...
		lock.setText("Adjust");
		lock.setToolTipText(
				"Ricalcola proporzionalmente gli ingredienti in base alle variazioni di volume o efficienza");
	...
	private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton2ActionPerformed
		...
		// frm.startModal(this);
		Double volbrewhouse;
		if (chkConcentratedBoil.isSelected()) {
			volbrewhouse = spinVolumeDiluito.getVolume();
		} else {
			volbrewhouse = spinVolumeFin.getVolume();
		}
	...
	private void btnBiabActionPerformed(java.awt.event.ActionEvent evt) {
		frmBiabUtils fbu = new frmBiabUtils(maltTableModel.getGrammiMash(), this.spinVolumeFin.getVolume(),
				((Integer) this.spinBollitura.getValue()).intValue(), fldNome.getText().trim());
	...
	private void btnAdd5ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnAdd5ActionPerformed
		EvaporationForm ed = new EvaporationForm(this, this.getVolumeBoll(), this.getVolume(), this.getGravity(),
				this.getBollitura());
		Gui.desktopPane.add(ed);
		Utils.center(ed, this);
		ed.setVisible(true);
	...
	private void btnAdd1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnAdd1ActionPerformed
		...
		if (type != null) {
			tblMalts.editCellAt(maltSorter.modelIndex(this.maltTableModel.addRow(new Malt(this.thisRicetta, type))), 2);
			ricettaModificata();
		}
	...
	private void formInternalFrameClosing(javax.swing.event.InternalFrameEvent evt) {// GEN-FIRST:event_formInternalFrameClosing
		...
		int option = JOptionPane.showConfirmDialog(Main.gui, "Uscire senza salvare le modifiche?", "Ricetta",
	...
	private void btnRemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnRemActionPerformed
		if (this.tblHops.getSelectedRow() < 0)
			return;
	...
	private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnAddActionPerformed
		...
		if (type != null) {
			...
			tblHops.editCellAt(hopSorter.modelIndex(this.hopTableModel.addRow(new Hop(this.thisRicetta, type))), 2);
			ricettaModificata();
		}
	...
	private void btnDupHopActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnDupHopActionPerformed
		if (this.tblHops.getSelectedRow() < 0)
			return;
		Hop h = Hop
				.fromXml(this.hopTableModel.getRow(this.hopSorter.modelIndex(this.tblHops.getSelectedRow())).toXml());
		h.setRicetta(this);
					...
					while (true) {
						while (Ricetta.this.counter > 0) {
							...
							if (Ricetta.this.counter == 0) {
								Ricetta.this.glassPanel.setColor(Ricetta.this.maltTableModel.getSRMMosher());
							}
						}
	...
	public void fromRecipeData(RecipeData src) {
		...
		// fatto alla cazzo, modificato metodo setBrewStyle, viene
		// scritto/recuperato da ricetta solo il codice (1A etc) e recuperato da
		// Main.getBJCPStyles
		...
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				splitPanel.setDividerLocation((double) (double) maltTableModel.getRowCount()
	...
	public void read(Document doc) {
		...
		splitPanel.setDividerLocation(
				(double) maltTableModel.getRowCount() / (hopTableModel.getRowCount() + maltTableModel.getRowCount()));
		this.dirty = false;
	}
	...
	public void setBrewStyle(String codiceStile) {
		...
		if (brewStyle != null) {
			...
			if (brewStyle.getOgMin() != null) {
				this.txtOG1.setText(
						nf.format(brewStyle.getOgMin() > 1000 ? brewStyle.getOgMin() / 1000 : brewStyle.getOgMin()));
			}
			if (brewStyle.getOgMax() != null) {
				this.txtOG2.setText(
						nf.format(brewStyle.getOgMax() > 1000 ? brewStyle.getOgMax() / 1000 : brewStyle.getOgMax()));
			}
			if (brewStyle.getFgMin() != null) {
				this.txtFG1.setText(
						nf.format(brewStyle.getFgMin() > 1000 ? brewStyle.getFgMin() / 1000 : brewStyle.getFgMin()));
			}
			if (brewStyle.getFgMax() != null) {
				this.txtFG2.setText(
						nf.format(brewStyle.getFgMax() > 1000 ? brewStyle.getFgMax() / 1000 : brewStyle.getFgMax()));
			}
	...
	public String getBUGUTinsethPerStampa() {
		...
		// hopTableModel.getIBUTinseth()/((this.maltTableModel.getSG(chkConcentratedBoil.isSelected())-1)*100));
		return String.format("%.2f", (hopTableModel.getIBUTinseth()
				/ (this.maltTableModel.getSG(chkConcentratedBoil.isSelected()) - 1) / 1000));
	}
	...
	public void finalizeInInventory() {
		...
		if (cantDoHop.size() > 0 || cantDoMalt.size() > 0) {
			...
			for (CanDo<Malt> m : cantDoMalt) {
				...
				if (m.getHave() != null)
					s += " (presenti " + m.getHave().getGrammi() + " gr, richiesti " + m.getReq().getGrammi() + ")";
			}
		...
		if (mayDoHop.size() > 0 || mayDoMalt.size() > 0) {
			...
			for (CanDo<Malt> m : mayDoMalt) {
				...
				if (tot >= m.getReq().getGrammi()) {
					...
					canDo = false;
					cantDo += "\n" + m.getReq().getNome() + " (presenti " + tot + " gr, richiesti "
							+ m.getReq().getGrammi() + ")";
				}
			}
			...
			if (canDo)
				new Info("Gli ingredienti sono tutti disponibili in inventario, seppure divisi in diverse voci.")
						.startModal(this);
			else
