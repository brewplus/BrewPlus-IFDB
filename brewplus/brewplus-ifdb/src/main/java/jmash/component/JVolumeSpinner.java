...
public class JVolumeSpinner extends JMultiUnitSpinner {
	...
	protected void changeUnit(int from, int to) {
		if (to == L) {
			if (from == GAL)
				setValues(Utils.galToLit(1.0));
			if (from == HL)
				setValues(100.0);
		} else if (to == GAL) {
			if (from == L)
				setValues(Utils.litToGal(1));
			if (from == HL)
				setValues(Utils.litToGal(100));
		} else if (to == HL) {
			if (from == GAL)
				setValues(Utils.galToLit(0.01));
			if (from == L)
				setValues(0.01);
		}
	}
	...
	public void setVolume(double V) {
		...
		if (i == L)
			...
			setDoubleValue(Utils.litToGal(V));
		else if (i == HL)
			setDoubleValue(V / 100);
	}
