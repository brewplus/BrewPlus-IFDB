...
public class JPressureSpinner extends JMultiUnitSpinner {
	...
	protected void changeUnit(int from, int to) {
		if (to == PSI) {
			if (from == BAR) {
				((SpinnerNumberModel) getSpinner().getModel()).setMaximum(
						Utils.BAR2PSI((Double) ((SpinnerNumberModel) getSpinner().getModel()).getMaximum()));
				((SpinnerNumberModel) getSpinner().getModel()).setMinimum(
						Utils.BAR2PSI((Double) ((SpinnerNumberModel) getSpinner().getModel()).getMinimum()));
				double T = getSpinner().getDoubleValue();
				getSpinner().setDoubleValue(Utils.BAR2PSI(T));
				((SpinnerNumberModel) getSpinner().getModel()).setStepSize(0.1);
			}
		} else if (to == BAR) {
			if (from == PSI) {
				((SpinnerNumberModel) getSpinner().getModel()).setMaximum(
						Utils.PSI2BAR((Double) ((SpinnerNumberModel) getSpinner().getModel()).getMaximum()));
				((SpinnerNumberModel) getSpinner().getModel()).setMinimum(
						Utils.PSI2BAR((Double) ((SpinnerNumberModel) getSpinner().getModel()).getMinimum()));
				double T = getSpinner().getDoubleValue();
	...
	public void setPressure(double V) {
		...
		if (i == PSI)
			...
			setDoubleValue(Utils.PSI2BAR(V));
	}
