...
public class JGravitySpinner extends JMultiUnitSpinner {
	...
	protected void changeUnit(int from, int to) {
		...
		if (to == SG) {
			...
			if (from == PL) {
				...
				getSpinner().setDoubleValue(precision == 0 ? Math.round(Utils.Plato2SG(T) * 1000) / 1000.0
	...
	public void setGravity(double V) {
		...
		if (i == SG)
			...
			getSpinner().setDoubleValue(Utils.SG2Plato(V));
		else if (i == BA)
			getSpinner().setDoubleValue(Utils.SG2Baume(V));
	}
	...
	public double getGravity() {
		...
		if (i == SG)
			...
			res = Utils.Plato2SG(T);
		else if (i == BA)
			res = Utils.Baume2SG(T);
		return res;
	}
}