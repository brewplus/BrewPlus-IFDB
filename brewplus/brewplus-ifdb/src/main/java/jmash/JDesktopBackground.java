...
public class JDesktopBackground extends JDesktopPane {
	...
	private void createScaledImage() {
		...
		if (cachedWidth > 0 && cachedHeight > 0) {
			...
			scaledImage.createGraphics().drawImage(image,
					AffineTransform.getScaleInstance(((float) cachedWidth) / image.getWidth(this),
							((float) cachedHeight) / image.getHeight(this)),
					null);
		} else
