...
public class WaterNeeded extends javax.swing.JInternalFrame {
    ...
    private static final long serialVersionUID = -5956568257889159432L;
    private static Logger LOGGER = Logger.getLogger(WaterNeeded.class);
    ...
    public WaterNeeded() {
        ...
        spnPreBoil.setModel(Main.getFromCache("WaterNeeded.rabboccoPreBoil", 0.0), 0, 1000000, 0.5, "0.0",
                "WaterNeeded.PB");
        spnPostBoil.setModel(Main.getFromCache("WaterNeeded.rabboccoPostBoil", 0.0), 0, 1000000, 0.5, "0.0",
                "WaterNeeded.PTB");
        spnAssorbimento.setModel(Main.getFromCache("WaterNeeded.coefficienteAssorbimento", 1.01), 0, 1000000, 0.001,
                "0.000", "WaterNeeded.ABS");
    ...
    public void fromXml(Element E) {
        ...
        if (E.getAttribute("spnBatchSize") != null)
            spnBatchSize.setVolume(new Double(E.getAttribute("spnBatchSize").getValue()));
        if (E.getAttribute("spnBoiltime") != null)
            spnBoiltime.setDoubleValue(new Double(E.getAttribute("spnBoiltime").getValue()));
        if (E.getAttribute("spnEvaporation") != null)
            spnEvaporation.setVolume(new Double(E.getAttribute("spnEvaporation").getValue()));
        if (E.getAttribute("spnKg") != null)
            spnKg.setDoubleValue(new Double(E.getAttribute("spnKg").getValue()));
        if (E.getAttribute("spnNeiGrani") != null)
            spnNeiGrani.setVolume(new Double(E.getAttribute("spnNeiGrani").getValue()));
        if (E.getAttribute("spnLitriKg") != null)
            spnLitriKg.setDoubleValue(new Double(E.getAttribute("spnLitriKg").getValue()));
        if (E.getAttribute("spnLosses") != null)
            spnLosses.setVolume(new Double(E.getAttribute("spnLosses").getValue()));
        if (E.getAttribute("spnTrub") != null)
            spnTrub.setVolume(new Double(E.getAttribute("spnTrub").getValue()));
        if (E.getAttribute("spnSparge") != null)
            spnSparge.setVolume(new Double(E.getAttribute("spnSparge").getValue()));
        if (E.getAttribute("spnMash") != null)
            spnMash.setVolume(new Double(E.getAttribute("spnMash").getValue()));
        if (E.getAttribute("spnShrink") != null)
            spnShrink.setDoubleValue(new Double(E.getAttribute("spnShrink").getValue()));
        if (E.getAttribute("spnPreBoil") != null)
            spnPreBoil.setVolume(new Double(E.getAttribute("spnPreBoil").getValue()));
        if (E.getAttribute("spnPostBoil") != null)
            spnPostBoil.setVolume(new Double(E.getAttribute("spnPostBoil").getValue()));
        if (E.getAttribute("spnAssorbimento") != null)
            spnAssorbimento.setVolume(new Double(E.getAttribute("spnAssorbimento").getValue()));
  ...
  // End of variables declaration//GEN-END:variables
    ...
    public void calc() {
        ...
        double res = (spnBatchSize.getVolume()) / ((100 - spnShrink.getDoubleValue()) / 100) + spnTrub.getVolume()
                + spnEvaporation.getVolume() * spnBoiltime.getDoubleValue() / 60 + spnLosses.getVolume()
    ...
    public void calc2() {
        ...
        double preBoil = (spnBatchSize2.getVolume()) / ((100 - spnShrink2.getDoubleValue()) / 100)
                + spnTrub2.getVolume() + spnEvaporation2.getVolume() * spnBoiltime2.getDoubleValue() / 60;
        ...
        LOGGER.debug("preBoil SG = "+preBoilSG);
        LOGGER.debug("preBoil Vol = "+preBoil);
        double TL = (preBoil + spnLosses2.getVolume());
        LOGGER.debug("devo produrre pero' "+TL);

        double F = 4.0;

        LOGGER.debug("voglio fare un mash con "+F+" L/kg");

        double KG = spnKg2.getWeight() / 1000;
        double ML = ((F - K) * KG) - spnLosses2.getVolume();
        LOGGER.debug("per cui posso recuperare dal mash "+ML+" litri");
        double MSG = (((preBoilSG - 1) * 1000) * preBoil / ML) / 1000 + 1;
        LOGGER.debug("che dovranno avere una SG di "+MSG);
