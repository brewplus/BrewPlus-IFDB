...
public class Gyle extends javax.swing.JPanel {
	...
	public Gyle(Ricetta ricetta) {
		 ...
		 * this.maltPicker.setIcon(new
		 * javax.swing.ImageIcon(getClass().getResource(
		 * "/jmash/images/malts.png"))); maltPicker.setFilters(new
		 * String[]{"Tutti","Grani","Estratto secco","Estratto liquido"}, null);
		 */
		...
		// tblHops.getColumnModel().getColumn(1).setCellEditor(new
		// SpinnerEditor());
		...
		// tblMalts.getColumnModel().getColumn(4).setCellRenderer(new
		// BarCellRenderer());
		...
		this.tblHops.getColumnModel().getColumn(0).setCellRenderer(new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable tblDataTable, Object value, boolean isSelected,
				...
				JLabel ret = (JLabel) super.getTableCellRendererComponent(tblDataTable, value, isSelected, hasFocus,
						markedRow, col);
				ret.setIcon(Main.hopIcon);
				ret.setText("");
				return ret;
			}
		});
	...
	private void spinVolumeFinWOStateChanged(javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_spinVolumeFinWOStateChanged
		...
		if (lock.isSelected()) {
			hopTableModel.adjustTo(getEfficienza(), v, getEfficienza(), getVolume());
			maltTableModel.adjustTo(getEfficienza(), v, getEfficienza(), getVolume());
		}
		setVolume(v);
		ricettaModificata();
	...
	private void btnAdd7ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnAdd7ActionPerformed
		Thread th = new Thread() {
			...
			public void run() {
				btnAdd7.setEnabled(false);
				UploadRicetta2 ed = new UploadRicetta2(ricetta.getXmlRicetta(), ricetta.toRecipeData().getNome(),
						ricetta.getIdRicettaHB());
	...
	private void btnAdd5ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnAdd5ActionPerformed
		EvaporationForm ed = new EvaporationForm(ricetta, this.volumeBoll, this.volume, this.getGravity(),
				this.getBollitura());
		Gui.desktopPane.add(ed);
		Utils.center(ed, ricetta);
		ed.setVisible(true);
					...
					while (true) {
						while (Gyle.this.counter > 0) {
							...
							if (Gyle.this.counter == 0) {
								Gyle.this.glassPanel.setColor(Gyle.this.maltTableModel.getSRMMosher());
							}
						}
