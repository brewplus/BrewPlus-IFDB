...
public class RecipeData {
    ...
    public void readRec(String filename) throws Exception {
        ...
        String[] maltType = new String[] { "?", "Grani", "Estratto Liquido", "Cristalli", "Fiocchi", "Altro2",
                "Altro3" };
        ...
        setNote("Notes: " + Utils.arr2String(b, 1157 + 635 * nH + 529 * nM + 589 * nE + 757) + "\nAward: "
                + Utils.arr2String(b, 1157 + 635 * nH + 529 * nM + 589 * nE + 757 + 4028));
        ...
        for (int i = 0; i < nE; i++) {
            ...
            H.setAlfaAcidi(0.0);
            H.setGrammi((double) (int) Utils
                    .ouncesToGrams(Utils.arr2Double(b, 1157 + 635 * nH + 529 * nM + 589 * i + 577)));
        ...
        if (!mashComplexMode) {
            ...
            String D[] = new String[] { "Acid rest", "Protein rest", "Intermediate rest", "Saccharification rest",
                    "Mash out", "Sparge" };
            for (int j = 0; j < 6; j++) {
                if (multistep || j >= 3) {
                    ...
                    step.setStartTemp(T);
                    step.setEndTemp((int) Math.round(Utils.F2C((int) 0xFF & Utils.arr2Byte(b, Y + i))));
        ...
        if (mashComplexMode) {
            LOGGER.debug(nS + " step:");
            int SD = 292;
            for (int i = 0; i < nS; i++) {
                int P = M + 267 + i * SD;
                LOGGER.debug("\t" + Utils.arr2String(b, P) + " infusionQ=" + Utils.arr2float(b, P + 280)
                        + " startT=" + Utils.arr2Byte(b, P + 256) + " stopT=" + Utils.arr2Byte(b, P + 260) + " infuseT="
                        + (Utils.arr2Byte(b, P + 264) & 0xFF) + " restTime=" + Utils.arr2Byte(b, P + 268) + " stepTime="
                        + Utils.arr2Byte(b, P + 272) + " type=" + Utils.arr2Byte(b, P + 255) + "");
            }
        }
    }
