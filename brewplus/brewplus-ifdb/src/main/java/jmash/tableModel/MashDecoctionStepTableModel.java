...
public class MashDecoctionStepTableModel extends GenericTableModel<MashStep> {
	...
	public MashDecoctionStepTableModel() {
		this.columnNames = new String[] { "Minuto", "T. iniz.", "L. 1' rampa", "T. 1' rest", "L. 1' rest",
				"L. 2' rampa", "T. 2' rest", "L. 2' rest", "L. mescolam.", "T. fin.", };
	}
	...
	String fieldNames[] = { "start", "startTemp", "decoctionRamp1", "decoctionRestT1", "decoctionRestL1",
			"decoctionRamp2", "decoctionRestT2", "decoctionRestL2", "decoctionRampDown", "endTemp", };

	private MashStep last = null;

	public MashStep getLast() {
		return this.last;
	}
	...
	public void remRow(int i) {
		if (i >= 0) {
			if ((i > 1) && (i < this.dataValues.size() - 1)) {
				(this.dataValues.get(i + 1)).setStartTemp((this.dataValues.get(i - 1)).getEndTemp());
			}
	...
	public XYSeriesCollection getDataSet(XYSeriesCollection ds) {
		...
		for (MashStep h : this.dataValues) {
			...
			{
				series2 = new XYSeries(h.getNome());
				map.put(h.getNome(), 0);
			}
			else
			{
				int count = map.get(h.getNome()) + 1;
				series2 = new XYSeries(h.getNome() +  "_" + count);
			...
			series2.add(h.getStart() + h.getDecoctionRamp1() + h.getDecoctionRestL1() + h.getDecoctionRamp2(),
					h.getDecoctionRestT2());
			series2.add(h.getStart() + h.getDecoctionRamp1() + h.getDecoctionRestL1() + h.getDecoctionRamp2()
					+ h.getDecoctionRestL2(), h.getDecoctionRestT2());
			series2.add(h.getStart() + h.getDecoctionRamp1() + h.getDecoctionRestL1() + h.getDecoctionRamp2()
					+ h.getDecoctionRestL2() + h.getDecoctionRampDown(), h.getEndTemp());
